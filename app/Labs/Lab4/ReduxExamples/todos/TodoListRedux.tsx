...
export default function TodoListRedux(){
  ...
  return (<div id="wd-todo-list-redux"><h2>Todo List (Redux)</h2>
    <ul className="list-group">
      <li className="list-group-item">
        ...
        <FormControl value={todo.title} onChange={(e)=> dispatch(setTodo({ ...todo, title:(e.target as HTMLInputElement).value }))} />
      </li>
      {todos.map((t:any)=>(<li key={t.id} className="list-group-item">
        <button id="wd-delete-todo-click" onClick={()=> dispatch(deleteTodo(t.id))}>Delete</button>
        <button id="wd-set-todo-click" onClick={()=> dispatch(setTodo(t))}>Edit</button>
        {t.title}
      </li>))}
    </ul><hr/></div>);
}